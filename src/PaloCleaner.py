...
class PaloCleaner:
    def __init__(self, report_folder, **kwargs):
        ...
        self._schedule_namesearch = dict()                      # Search datastructure which permits to find all panos.objects.ScheduleObject  by its name (per device-group)
...
""")
    ...
    def fetch_objects(self, context, location_name):
        ...
        if location_name == 'predefined':
            ...
            self._schedule_namesearch[location_name] = dict()
            ...
            # download all ScheduleObject and add it to the Schedules key, + populate name search structure
            self._objects[location_name]['Schedule'] = ScheduleObject.refreshall(context)
            self._schedule_namesearch[location_name] = {x.name: x for x in self._objects[location_name]['Schedule']}
            self._console.log(f"[ {location_name} ] Schedules namesearch structures initialized", level=2)
    ...
    def get_relative_object_location(self, obj_name, reference_location, obj_type="Address", find_all=False, iterative_call=False):
        ...
        if obj_type == "Address":
            ...
            found_object = self._schedule_namesearch[reference_location].get(obj_name, None)
    ...
    def flatten_object(self, used_object: panos.objects, object_location: str, usage_base: str, referencer_type: str = None, referencer_name: str = None, resolved_cache=None):
        ...
        def flatten_object_recurser(used_object: panos.objects, object_location: str, usage_base: str, referencer_type: str = None, referencer_name: str = None, recursion_level: int = 1, protect_call=False):
            ...
            if type(used_object) in [panos.objects.AddressObject, panos.objects.ServiceObject, panos.objects.Tag, panos.objects.ScheduleObject]:
                self._console.log(
            ...
            if not isinstance(used_object, type(None)):
                if type(used_object) not in [panos.objects.Tag, panos.objects.ScheduleObject]:
                    if used_object.tag:
    ...
    def fetch_used_obj_set(self, location_name, progress, task):
        ...
        resolved_cache = dict({'Address': dict(), 'Service': dict(), 'Tag': dict(), 'Schedule': dict()})
        ...
        for k, v in self._rulebases[location_name].items():
            ...
            for r in v:
                ...
                if self._parse_schedules and (rule_schedule_name := getattr(r, "schedule", None)):
                    rule_schedule_object, _ = self.get_relative_object_location(rule_schedule_name, location_name, "Schedule")
                    if PaloCleanerTools.is_over_schedule(rule_schedule_object):
                        self._console.log(
                            f"  * Expired schedule on rule {r.name!r} ({rule_schedule_name})",
                            style="red")
                    if self._apply_cleaning:
                        self._console.log(
                            f" Deleting rule... "
                        ...
                        try:
                            # Extract rulebase class from key (e.g., "PreRulebase_SecurityRule" -> "PreRulebase")
                            rulebase_class_name = k.split("_")[0]
                            rulebase_classes = {"PreRulebase": PreRulebase, "PostRulebase": PostRulebase, "Rulebase": Rulebase}
                            rulebase = rulebase_classes[rulebase_class_name]()
                            # Build proper hierarchy: context -> rulebase -> rule
                            self._rulebases[location_name]['context'].add(rulebase)
                            rulebase.add(r)
                            r.delete()
                            # We don't consider the rule objects as used, as this rule is supposed to be deleted (schedule expired)
                            continue
                        except Exception as e:
                            self._console.log(f"  ERROR DELETING RULE : {e}, continuing")
                    else:
                        # We don't consider the rule objects as used, as this rule is supposed to be deleted (schedule expired)
                        continue
    ...
    def optimize_objects(self, location_name: str, progress: rich.progress.Progress, task: rich.progress.TaskID):
        ...
        # Remove ScheduleObject from the current context (to be reviewed, not sure) 
        rem_sched = [x for x in self._objects[location_name]['context'].children if type(x) is ScheduleObject]
        any(self._objects[location_name]['context'].remove(x) for x in rem_sched)
    ...
    def replace_object_in_rulebase(self, location_name: str, progress: rich.progress.Progress, task: rich.progress.TaskID):
        ...
        def replace_in_rule(rule, editable_rule):
            ...
            for obj_type in repl_map.get(type(rule)):
                ...
                # We don't replace schedules. We just want to analyze them to detect outdated schedules and associated rules, and delete unused ones
                            ...
                            if replacements_count:
                                ...
                                for obj_type, fields in repl_map[type(r)].items():
                                    for f in [x[0] if type(x) is list else x for x in fields]:
                                        ...
                                        if field_values:
                                            for object_name in field_values:
                                                if object_name in self._replacements[location_name].get(obj_type, []):
                                                    if not editable_rule or "noopstate" in r.name:
    ...
    def clean_local_object_set(self, location_name: str):
        ...
        #}
        clean_types = list(set([key for nested_dict in cleaning_order.values() for key in nested_dict]))
        self._cleaning_counts[location_name] = {
            x: {'removed': 0, 'replaced': 0} for x in clean_types
        }
